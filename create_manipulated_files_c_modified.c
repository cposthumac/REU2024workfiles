...
void insert_dead_code(Elf_Manager* manager) {
    ...
    // Loop through sections to find .main section
    for (int i = 0; i < manager->e_hdr.e_shnum; ++i) {
        if (strcmp(manager->s_hdr[i].sh_name + manager->file_sections[i], ".main") == 0) {
            ...
            for (uint64_t j = start; j < end - 2; ++j) {
                if (manager->file_sections[i][j] == 0x55 && manager->file_sections[i][j + 1] == 0x48 &&
                    manager->file_sections[i][j + 2] == 0x89 && manager->file_sections[i][j + 3] == 0xE5) {
...
void modify_bss_section(Elf_Manager* manager) {
    ...
    for (int i = 0; i < manager->e_hdr.e_shnum; ++i) {
        if (strcmp(manager->s_hdr[i].sh_name + manager->file_sections[i], ".bss") == 0) {
            uint64_t start = manager->s_hdr[i].sh_offset;
            uint64_t end = start + manager->s_hdr[i].sh_size;

            // Extend the .bss section size by setting the file sections to 0xFF
